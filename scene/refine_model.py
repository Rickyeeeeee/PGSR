...
class RefineImgGaussianModel:
    # Fixed parameters:
    # base_xyz, base_scaling, base_rotation, base_opacity
    # Optimized parameters:
    # features_dc, features_rest, opacity
    def __init__(self, gs_model: GaussianModel):
        ...
        self._base_scaling = gs_model._scaling.detach().clone().requires_grad_(False).to("cuda")
        self._base_rotation = gs_model._rotation.detach().clone().requires_grad_(False).to("cuda")
        self._base_opacity = gs_model._opacity.detach().clone().requires_grad_(False).to("cuda")
    ...
    def capture(self):
        return (
            ...
            self._base_xyz,
            self._base_features_dc,
            self._base_features_rest,
            self._base_scaling,
            self._base_rotation,
            self._base_opacity,
            self._base_count,
