...
class DualGaussianModel:
    def __init__(self, gs_model: GaussianModel):
        ...
        self._dual_opacity = gs_model._opacity.detach().clone().requires_grad_(True).to("cuda")

        self._app_xyz = torch.empty(0)
        self._app_features_dc = torch.empty(0)
        self._app_features_rest = torch.empty(0)
        self._app_scaling = torch.empty(0)
        self._app_rotation = torch.empty(0)
        self._app_opacity = torch.empty(0)
    ...
    def named_parameters(self):
        return [
            ...
            ("dual_opacity", self._dual_opacity)
        ]
    ...
    @property
    def get_dual_opacity(self):
        return self.opacity_activation(self._dual_opacity)
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._dual_opacity], 'lr': training_args.opacity_lr, "name": "dual_opacity"},
    ...
    def prune_points(self, mask):
        ...
        self._opacity = optimizable_tensors["opacity"]
        self._dual_opacity = optimizable_tensors["dual_opacity"]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_dual_opacities, new_scaling, new_rotation):
        d = {"xyz": new_xyz,
        "f_dc": new_features_dc,
        "f_rest": new_features_rest,
        "opacity": new_opacities,
        "dual_opacity": new_dual_opacities,
        "scaling" : new_scaling,
    ...
    def densify_and_split(self, grads, grad_threshold, grads_abs, grad_abs_threshold, scene_extent, max_radii2D, N=2):
        ...
        new_dual_opacity = self._dual_opacity[selected_pts_mask].repeat(N,1)

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_dual_opacity, new_scaling, new_rotation)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        if selected_pts_mask.sum() > 0:
            ...
            new_dual_opacities = self._dual_opacity[selected_pts_mask]
    ...
    def increment_denom(self, update_filter):
